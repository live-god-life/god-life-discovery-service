pipeline {
    agent any
    environment {
        imagename = "onezo/god-life-discovery-service"
    ...
    stages {
        stage('git clone') {
            steps {
                ...
                git url: 'git@github.com:live-god-life/god-life-discovery-service.git',
                branch: 'master',
        ...
        stage('Run Docker Deploy Script') {
            steps {
                ...
                sh "ssh -o StrictHostKeyChecking=no root@106.10.40.232 -p 2222 '/root/DeployScript/discovery-service-deploy.sh'"
            }
            post {
                success {
